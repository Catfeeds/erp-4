...
@section('content')
    ...
    <div>
        ...
        <ul class="nav nav-tabs" role="tablist">
            <li role="presentation" class="active"><a href="#groups" aria-controls="groups" role="tab" data-toggle="tab">用户组</a></li>
        ...
        <div class="tab-content">
                ...
                <div class="row">
                    <div class="col-lg-6">
                        <div class="alert alert-info">
                            <i class="fa fa-info-circle"></i> 拖动用户组来组织你的分组。不管分组如何变化，权限仍然分别分配给每个角色。
                        </div><!--alert info-->
                            ...
                            <ol class="dd-list">
                                @foreach ($groups as $group)
                                    <li class="dd-item" data-id="{!! $group->id !!}">
                                        <div class="dd-handle">{!! $group->name !!} <span class="pull-right">{!! $group->permissions->count() !!} 个权限</span></div>

                                        @if ($group->children->count())
                                            <ol class="dd-list">
                                                @foreach($group->children as $child)
                                                    <li class="dd-item" data-id="{!! $child->id !!}">
                                                        <div class="dd-handle">{!! $child->name !!} <span class="pull-right">{!! $child->permissions->count() !!} 个权限</span></div>
                                                    </li>
                                                @endforeach
                                            </ol>
                                        </li>
                    ...
                    <div class="col-lg-6">
                        <div class="alert alert-info">
                            <i class="fa fa-info-circle"></i>层次结构变化后，需要刷新页面才能得新的组织结构。
                        </div><!--alert info-->
            ...
            <div role="tabpanel" class="tab-pane" id="permissions" style="padding-top:20px">
                ...
                <div class="pull-left">
                    {{ trans('crud.permissions.total') }} : {{ $permissions->total() }} 
                </div>
                ...
                @permission('sort-permission-groups')
                    $.ajax({
                        ...
                        success: function(data) {
                            if (data.status == "OK")
                                toastr.success("层次结构保存成功。");
                            else
                                toastr.error("出现未知错误。");
                        },
                        error: function (jqXHR, textStatus, errorThrown) {
                            toastr.error("发生未知错误：" + errorThrown);
                        }
                    });
                @else
                    toastr.error("您没有权限。");
