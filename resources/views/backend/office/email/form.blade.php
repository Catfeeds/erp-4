<form class="form-horizontal" action="{{ $form['url'] }}" method="{{ $form['method'] }}" >
  ...
  <div class="form-group{!! ($errors->has('title')) ? ' has-error' : '' !!}">
    <label name="name" class="control-label col-md-2">主题</label>
    <div class="col-md-8">
      <input name="title" class="form-control" value="{!! Request::old('title', $form['defaults']['title']) !!}" placeholder="主题" type="text">
    ...
    <div class="col-md-8"> 
      <select  name="to" class="select"> 
        @foreach ($users as $content)
          <option value="{!! Request::old('to', $content->id); !!}" type="text">{{ $content->name }}</option>
        @endforeach
      </select> 
